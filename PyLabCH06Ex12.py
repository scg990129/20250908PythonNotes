...
for monthName in months:
    if (l := len(monthName)) > monthLength:
        monthLength = l
...
try:
    with open(stepFile, "r") as file:
        ...
        for line in lines:
            if not (line := line.strip()).isnumeric() and int(line) != Decimal(line): raise ValueError("Please check steps.txt file: contain non-integer value.")
